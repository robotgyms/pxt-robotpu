...
\begin{minipage}[t]{0.49\linewidth}
  ...
  From elementary classrooms to high-school robotics clubs, Robot PU turns
  abstract STEM concepts into hands-on, creative projects.
  Two ready-to-use software options are available: a beginner track
  for younger students and an advanced track with more sensors,
  actions, and AI-camera projects. Add the optional \textbf{Smart Hat}
  to unlock face tracking, robot soccer, and mapping.}
  ...
  % Optional add-on
  \featurebox{puOrange}{\faMicrochip\; Optional: Smart Hat for AI \& Soccer}{%
    Adds AI camera, voice commands, mapping, face detection,
    object tracking, and robot soccer. \textit{Sold separately.}
  }
\end{minipage}
...
\begin{multicols}{4}
  ...
  \end{center}
  {\small\bfseries\color{puDark}Move \& Express}\\
  {\footnotesize
    Walk, turn, step sideways, kick, jump, and rest.
    Dance to music. Talk and sing with a friendly robot voice.
    Blinking LED eyes show mood and personality.}
  ...
  \end{center}
  {\small\bfseries\color{puDark}Explore \& Navigate}\\
  {\footnotesize
    Roams around obstacles, solves simple mazes,
    follows a line or path, and can trail a leader.
    Great for teaching problem solving and spatial reasoning.}
  ...
  \end{center}
  {\small\bfseries\color{puDark}Sense \& Balance}\\
  {\footnotesize
    Uses distance and tilt sensors to stay upright,
    avoid bumps, and react to sounds.
    Self-balancing motion keeps it steady on the desk or floor.}
  ...
  \end{center}
  {\small\bfseries\color{puDark}Teamwork \& AI}\\
  {\footnotesize
    Robots can chat by radio, sing together, and play team games.
    Add the Smart Hat camera to try face recognition,
    object tracking, and robot soccer.}
...
\featurebox{puBlue}{\faWrench\; Build \& Calibration}{%
  ...
  \begin{tabular}{@{}p{0.06\linewidth}p{0.78\linewidth}@{}}
    ...
    {\color{puYellow}$\bigstar$}   & Material specifications \\
  \end{tabular}}
}

\vspace{3pt}

\featurebox{puBlue}{\faRunning\; Robot Hardware \& Behaviors}{%
  {\footnotesize
  \begin{tabular}{@{}p{0.06\linewidth}p{0.78\linewidth}@{}}
...
\featurebox{puOrange}{\faCodeBranch\; Programming Structure}{%
  ...
  \begin{tabular}{@{}p{0.06\linewidth}p{0.78\linewidth}@{}}
    ...
    {\color{puYellow}$\bigstar\bigstar$} & State switch (start/stop) \\
    {\color{puYellow}$\bigstar\bigstar$} & Object-oriented design \\
  \end{tabular}}
}
...
% ---- Column 3 ----------------------------------------------------------------
\featurebox{puGray}{\faSitemap\; Advanced Systems}{%
  {\footnotesize
  \begin{tabular}{@{}p{0.06\linewidth}p{0.78\linewidth}@{}}
    {\color{puYellow}$\bigstar\bigstar$} & Robots as different roles \\
    {\color{puYellow}$\bigstar\bigstar$} & Sharing messages between robots \\
    {\color{puYellow}$\bigstar\bigstar$} & Move commands \\
    {\color{puYellow}$\bigstar\bigstar$} & Sense the surroundings \\
    {\color{puYellow}$\bigstar\bigstar$} & Health checks \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Activity logging \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Teacher dashboard \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Safety stop + E-stop \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Recovering from faults \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Behavior plans \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Mission plans \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Sharing tasks \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Team formations \\
  \end{tabular}}
}

\vspace{3pt}

\featurebox{puGray}{\faCalculator\; Math \& Stability}{%
  {\footnotesize
  \begin{tabular}{@{}p{0.06\linewidth}p{0.78\linewidth}@{}}
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & How math makes the robot move \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Self-correcting motion \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Smoothing sensor readings \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Reliable direction sensing \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Balance on two legs \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Balance on uneven ground \\
  \end{tabular}}
}

\vspace{3pt}

\featurebox{puOrange}{\faCamera\; AI Camera \& Robot Soccer}{%
  {\footnotesize
  \begin{tabular}{@{}p{0.06\linewidth}p{0.78\linewidth}@{}}
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Smart Hat AI camera \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Find and follow a face \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Read smart-camera messages \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Track where the robot goes \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Follow a soccer ball \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Build a local map \\
    {\color{puYellow}$\bigstar\bigstar\bigstar$} & Keep a 1~m social distance \\
  \end{tabular}}
}
